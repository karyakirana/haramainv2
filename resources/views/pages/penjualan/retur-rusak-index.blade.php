<x-metronics-layout>
    ...
    <x-organism.card :title="__('Penjualan')">
        <x-slot name="header">
            <button type="button" class="btn btn-primary align-self-center" onclick="returrusakAdd()">New Data</button>
        </x-slot>
    ...
    @push('custom-scripts')
        <script>
            ...
            function returrusakAdd()
            {
                window.location.href = "{{route('returrusak.create')}}";
            }

            function edit(id)
            {
                windows.location.href = "{{url('/').'retur/rusak/edit/'}}"+id;
            }
