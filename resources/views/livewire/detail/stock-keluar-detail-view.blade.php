<div>
    <x-organism.modal :title="__('Detail Retur')" :tipe="__('xl')" id="detailModal"  wire:ignore.self>
        <form id="detailModal">
            ...
            <div class="form-group row">
                <label class="col-form-label col-3">Tgl Keluar</label>
                <div class="col-4">
                    <p class="form-control-plaintext">{{  tanggalan_format($tgl_keluar) ?? ''}}</p>
                </div>
        ...
        <table class="table table-row-bordered table-bordered">
            ...
            <tbody class="border">
            @isset($detailStockKeluar)
                @forelse($detailStockKeluar as $row)
                    <tr class="border">
